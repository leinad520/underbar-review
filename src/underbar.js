(function() {
  ...
  _.uniq = function(array, isSorted, iterator) {
    let temp = []; //[true, true, false...] original array = [1,1,3,4,5]
    let temp2 = []; //[true, false]
    let result = [];
    if (iterator) {
      for (let el of array) {
        temp.push(iterator(el))
      }
      ...
      for (let i = 0; i<temp2.length; i++) {
        let index = temp.indexOf(temp2[i]);
  ...
  _.extend = function(obj) { //obj={a:1} {a:2, b:2}
      //if (!obj.length) return obj;
      
      for (let i = 1; i < arguments.length; i++) {
        let object = arguments[i];
        for (let key in object) {
          arguments[0][key] = object[key];
        }
      }
    return arguments[0];;
  };
